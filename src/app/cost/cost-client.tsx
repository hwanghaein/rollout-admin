...
export default function CostClient({ costMenuList }: CostClientProps) {
  ...
  // 메뉴 데이터를 계산하여 상태에 저장
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(fireStore, "costMenuItems"), (snapshot) => {
      ...
      const updatedMenus = fetchedMenus.map((menu) => {
        ...
        const costPerPiece = calculateCostPerPiece(totalCost, menu.salesQuantity);
        const margin = calculateMargin(menu.pricePerPiece, costPerPiece);
        const profitPerPiece = calculateProfitPerPiece(menu.pricePerPiece, costPerPiece);
    ...
    <>
      <div>
        ...
        <div className="flex justify-end mb-4">
          <button
            className="px-2 py-1 cursor-pointer bg-blue-500 text-white rounded text-sm"
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm text-left text-dark2">
            <thead className="bg-gray-300">
              <tr>
                <th className="px-3 py-2 font-bold text-dark2 border-x border-gray-200">메뉴명</th>
                <th className="px-3 py-2 font-bold text-dark2 border-x border-gray-200">총 원가</th>
                <th className="px-3 py-2 font-bold text-dark2 border-x border-gray-200">판매가</th>
                <th className="px-3 py-2 font-bold text-dark2 border-x border-gray-200">마진율</th>
              </tr>
            </thead>
            <tbody>
              {calculatedMenus.map((item) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  {item.totalCost === 0 ? "0원" : `${item.totalCost.toFixed(2)}원`}

                  </td>
                  <td className="px-3 py-2 border-x border-gray-200">{item.pricePerPiece}원</td>
                  <td className="px-3 py-2 border-x border-gray-200">
        ...
        <div className="mt-5 text-sm text-gray-600">
          총 {calculatedMenus.length}개의 메뉴가 등록돼 있습니다.
        </div>
      </div>
    </>
  );
